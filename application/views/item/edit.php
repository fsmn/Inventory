...
<form id="item_editor" class="form-horizontal" action="<?=site_url("item/$action")?>" method="post" name="item_editor">
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			e-
			<input type="text" class="form-control" name="po" id="po" required style="width: 4em" value="<?=get_value($item, 'po', $po);?>" />
		</div>
	</div>
	<div class="form-group">
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="text" class="form-control" name="sku" required id="sku" style="width: auto" value="<?=get_value($item, 'sku'); ?>" />
		</div>
	</div>
...
<?=form_dropdown('category', $categories, urlencode(get_value($item, 'category')), 'id="category" class="form-control"');?>
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="text" class="form-control" required name="price" id="price" style="width: 5em" value="<?=get_value($item, 'price')?>" />
		</div>
	</div>
	<div class="form-group">
		<label class="col-sm-4 control-label" for="total">Total: </label>
		<div class="col-sm-8">
			<input type="text" class="form-control" name="total" readonly id="total" style="width: 5em" value="<?=$total?>" />
		</div>
	</div>
	<div class="form-group">
		<div class="col-sm-8 col-sm-offset-4">
			<input type="submit" class="button save_item <?=$action;?> <?=implode(" ",get_button_style($action));?>" value="<?=ucfirst($action);?>" />
		</div>
	</div>
</form>
